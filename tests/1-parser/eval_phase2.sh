...
function run_tests() {
    ...
    for testcase in "$TESTCASE_DIR"/*.cx; do
        ...
        # 生成学生的 AST 文件
        "$BUILD_DIR"/cxfc -emit-ast "$testcase" > "$OUTPUT_DIR/${filename%.cx}.ast"
        ...
        if [[ ${2:-no} != "no" ]]; then
            ...
            if [[ ${2:-no} == "verbose" ]]; then
                diff "$OUTPUT_DIR/${filename%.cx}.ast" "$OUTPUT_STD_DIR/${filename%.cx}.ast"
            else
                diff -q "$OUTPUT_DIR/${filename%.cx}.ast" "$OUTPUT_STD_DIR/${filename%.cx}.ast"
            fi
...
if [[ $1 == "-all" ]]; then
    ...
    for TESTCASE in "${TESTCASES[@]}"; do
        ...
        VALID_FILES=$(find "$TESTCASE_DIR" -maxdepth 1 -name '*.cx' ! -name 'FAIL*' | wc -l)
