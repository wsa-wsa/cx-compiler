...
function run_tests() {
    ...
    # 对每个 .cx 文件生成 syntax_tree 文件
    for testcase in "$TESTCASE_DIR"/*.cx; do
        filename="$(basename "$testcase")"

        echo "[info] Analyzing $filename"
        
        # 生成学生的 syntax_tree 文件
        "$BUILD_DIR"/parser "$testcase" > "$OUTPUT_DIR/${filename%.cx}.syntax_tree"
        ...
        if [[ ${2:-no} != "no" ]]; then
            ...
            if [[ ${2:-no} == "verbose" ]]; then
                diff "$OUTPUT_DIR/${filename%.cx}.syntax_tree" "$OUTPUT_STD_DIR/${filename%.cx}.syntax_tree"
            else
                diff -q "$OUTPUT_DIR/${filename%.cx}.syntax_tree" "$OUTPUT_STD_DIR/${filename%.cx}.syntax_tree"
            fi
...
if [[ $1 == "-all" ]]; then
    ...
    for TESTCASE in "${TESTCASES[@]}"; do
        ...
        # 计算有效文件数
        VALID_FILES=$(find "$TESTCASE_DIR" -maxdepth 1 -name '*.cx' | wc -l)
