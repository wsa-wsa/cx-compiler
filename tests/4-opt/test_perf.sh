...
for case in $testcases; do
    ...
    #### mem2reg off
    # cxfc compile to .s
    bash -c "${project_dir}/build/cxfc -S $nflag $case -o $asm_mem2reg_off" >>$LOG 2>&1
    check_return_value $? 0 "CE" "cxfc compiler error" || continue
    ...
    #### mem2reg on
    # cxfc compile to .s
    bash -c "${project_dir}/build/cxfc -S $flag $case -o $asm_mem2reg_on" >>$LOG 2>&1
    check_return_value $? 0 "CE" "cxfc compiler error" || continue
    # gcc compile asm to executable
    loongarch64-unknown-linux-gnu-gcc -static \
