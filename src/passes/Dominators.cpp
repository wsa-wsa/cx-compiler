...
void Dominators::create_idom(Function *f) {
    ...
    auto &bb_list = post_order_vec_;
    auto entry = f->get_entry_block();
    idom_[entry] = entry;
    bool changed = true;
    while (changed) {
        changed = false;
        for (auto itbb =bb_list.rbegin(); itbb!=bb_list.rend(); ++itbb) {
            auto bb = *itbb;
            if (bb == entry) {
                continue;
            }
...
void Dominators::create_dominance_frontier(Function *f) {
    ...
    for (auto &bb1 : f->get_basic_blocks()) {
        ...
        for (auto &pred : bb->get_pre_basic_blocks()) {
            auto runner = pred;
            // 如果当前控制块bb的直接支配节点不是该前向控制块pred
            // 则把前向控制块pred的支配边界设置为bb,并继续向上搜索直到到达直接支配节点
